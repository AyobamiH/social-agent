...
async function main() {
    ...
    console.log('\n2. Facebook Pages...');
    const pages = await get('graph.facebook.com', `/${graphVersion}/me/accounts?fields=id,name,access_token&access_token=${token}`);
    if (pages.error) {
    ...
    if (!igId) {
        const configuredPage = pages.data?.find(page => page.id === config_1.default.FACEBOOK_PAGE_ID);
        if (linkedIgAccounts.length && configuredPage) {
            console.log(`   ✓ INSTAGRAM_ACCOUNT_ID not set, but it can be auto-discovered from FACEBOOK_PAGE_ID ${configuredPage.id}`);
        }
        else if (linkedIgAccounts.length && config_1.default.FACEBOOK_PAGE_ID) {
            const discoveredPages = linkedIgAccounts.map(account => account.pageId).join(', ');
            console.log(`   ✕ INSTAGRAM_ACCOUNT_ID not set, and FACEBOOK_PAGE_ID ${config_1.default.FACEBOOK_PAGE_ID} does not match the Page(s) currently linked to Instagram: ${discoveredPages}`);
        }
        else if (config_1.default.FACEBOOK_PAGE_ID) {
            console.log('   ✕ INSTAGRAM_ACCOUNT_ID not set in .env and no page-linked instagram_business_account is available yet');
        }
        else {
    ...
    else if (pages.data?.some(page => page.id === config_1.default.FACEBOOK_PAGE_ID && page.access_token)) {
